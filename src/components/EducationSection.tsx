...
const modules = [
  { id: '01', title: 'Financial Market Fundamentals', status: 'Available', active: true, meta: '4 Lessons • Free' },
  { id: '02', title: 'Technical Analysis and Chart Reading', status: 'Coming Soon', active: false, meta: '8 Lessons' },
  { id: '03', title: 'Algorithmic Trading with MQL5', status: 'Coming Soon', active: false, meta: '12 Lessons' },
];

const lessons = [
  { title: 'What is the Financial Market?', active: true, image: eduMercado },
  { title: 'Market Participants', active: true, image: eduFracoes },
  { title: 'Financial Assets and Classes', active: false, image: eduReinvestimentos },
  { title: 'How the Stock Exchange Works', active: false, image: null },
];
    ...
    <div className="max-w-7xl mx-auto px-6 relative z-10">
      <div className="mb-16 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div>
          ...
          <h2 className="font-montserrat text-3xl font-bold text-foreground tracking-[-0.02em]">Learn to Trade with Method</h2>
        </div>
        <img src={savingsEducation} alt="Financial Education" className="w-28 h-28 object-contain rounded-xl hidden md:block" />
      </div>
        ...
        {modules.map((mod) => (
          <div key={mod.id} className={`p-8 rounded-xl border transition-all shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)] ${mod.active ? 'bg-surface-mid border-primary/20' : 'bg-surface-mid/40 border-foreground/5 opacity-60'}`}>
            <div className="flex justify-between items-start mb-6">
              <span className="font-montserrat text-primary font-bold">Module {mod.id}</span>
            ...
            >
              {mod.active ? 'Access Module' : 'Locked'}
            </button>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-10">
        <img src={eduMercado} alt="Financial Market" className="w-full h-44 object-cover rounded-xl border border-foreground/5" />
        <img src={eduFracoes} alt="Fractions and Assets" className="w-full h-44 object-cover rounded-xl border border-foreground/5" />
        <img src={eduReinvestimentos} alt="Reinvestments" className="w-full h-44 object-cover rounded-xl border border-foreground/5" />
      </div>
      ...
      <div className="bg-surface-mid rounded-2xl overflow-hidden border border-foreground/5 shadow-[inset_0_1px_1px_rgba(255,255,255,0.05)]">
        <div className="p-6 border-b border-foreground/5 bg-foreground/5">
          <h4 className="font-montserrat font-bold text-foreground text-sm">Module 01 Content</h4>
        </div>
