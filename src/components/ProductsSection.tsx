...
import { Bot, ShieldCheck, TrendingUp, Layers, SplitSquareHorizontal, Phone, Mail, Play, Lock, GraduationCap, Cpu } from 'lucide-react';
...
const tabs = [
  ...
  { id: 'produtos', label: 'Produtos', icon: Cpu },
];
        ...
        <div className="flex gap-2 mb-12 justify-center">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-lg font-montserrat font-bold text-sm transition-all ${
                activeTab === tab.id
                  ? 'bg-primary text-primary-foreground shadow-[0_4px_20px_hsl(var(--primary)/0.25)]'
                  : 'bg-card border border-foreground/10 text-muted hover:text-foreground hover:border-foreground/20'
              }`}
            >
              <tab.icon size={18} />
              {tab.label}
